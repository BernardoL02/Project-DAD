...
onMounted(async () => {
  storeAuth.socket.on('connect', () => {
    if (storeAuth.user && storeGameMultiplayer.activeGames.length) {
      const gameId = storeGameMultiplayer.activeGames[0].id
      storeAuth.socket.emit('updateSocketIdInGame', storeAuth.user.id, gameId)
    }
  })
    ...
    <div class="players-info p-4 bg-gray-50 w-64 rounded-lg">
      ...
      <ul>
        <li v-for="(player, index) in gameData.players" :key="player.id"
          class="flex items-center gap-4 mb-4 p-2 rounded-lg transition-transform duration-300 relative" :class="{
          ...
          <div class="relative w-12 h-12">
            ...
            <svg v-if="index === gameData.currentPlayerIndex" class="absolute inset-0 w-12 h-12 z-10"
              ...
              <circle cx="20" cy="20" r="18" fill="none" stroke="#4caf50" stroke-width="4" :style="circleStyle"
          ...
          <div v-if="player.inactive" class="absolute inset-0 flex items-center justify-center text-red-500 font-bold">
            Leave
          </div>
        </li>
      </ul>
