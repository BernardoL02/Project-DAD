...
const columns = ['Name', 'Email', 'NickName', 'Type']
const PlayerTypeOptions = ['All', 'Administrator', 'Player']
const AccountTypeOptions = ['All', 'Blocked', 'Unblocked', 'Deleted']
...
<template>
  <div class="max-w-5xl mx-auto py-8 space-y-6">
    <div class="flex items-center justify-between w-full pl-40">
      <h1 class="text-3xl font-bold text-center flex-grow mb-2">User Management</h1>
      <button @click="openRegisterModal" class="py-1 px-4 bg-sky-500 text-white rounded hover:bg-sky-600 ml-8 mt-2">
        Register Admin
      </button>
    </div>
    ...
    <div class="bg-white p-6 rounded-lg shadow-md mb-6">
      <div class="flex flex-col sm:flex-row sm:justify-between gap-5">
        <div class="w-full sm:w-auto">
          ...
          <input type="text" id="search" v-model="adminStore.searchTerm" placeholder="Search by Nickname or Email"
        ...
        <div class="w-full sm:w-auto">
          ...
          <DropdownButton :options="AccountTypeOptions" v-model="adminStore.selectedStatus"
        ...
        <div class="w-full sm:w-auto">
          ...
          <DropdownButton :options="PlayerTypeOptions" v-model="adminStore.selectedType"
    ...
    <div>
      ...
      <div v-else>
        <PaginatedTable :columns="columns" :data="adminStore.filteredUsers"
          :hidden-columns="['Blocked', 'Id', 'Deleted']" :pagination="true">
          <template #actions="{ row }">
            <div v-if="row.Deleted == 'NotDeleted'" class="flex space-x-2 justify-center">
              <button v-if="row.Blocked === 'Unblocked'" @click="openModal('block', row.Id, row.NickName)">
            ...
            <div v-else class="flex space-x-2 justify-center text-white bg-red-400 rounded-xl">
              Deleted
            </div>
          </template>
        </PaginatedTable>
