...
const startGame = async (size, cost, board_id) => {
  ...
  if (authStore.user) {
    ...
    try {
      gameId = await gameStore.createSinglePlayer(
        board_id,
        difficulty.value === 'hard' ? 'hard' : null
      )
...
<template>
  <div class="max-w-3xl mx-auto py-8 space-y-6">
    ...
    <div v-if="authStore.user" class="bg-sky-100 p-6 rounded-xl shadow-lg flex justify-between items-center">
      <div>
    ...
    <div class="bg-white p-4 rounded-xl shadow-lg">
      <div class="relative text-center mb-6 mr-4">
        ...
        <div class="group ml-0 cursor-pointer absolute right-0 top-0 transform translate-x-4 translate-y-1 mr-2">
              ...
              <div class="mt-3 grid grid-cols-1">
                <div>
                  <p class="text-sm font-semibold text-gray-700">
                    Normal -
                    <span class="text-sm font-normal text-gray-500"> Find pairs of cards. </span>
                  </p>
                  <p class="text-sm font-semibold text-gray-700">
                    Hard -
      ...
      <div class="flex items-center space-x-2 flex-row justify-center mb-5 mt-2">
        <div
          class="relative w-32 h-8 bg-gray-200 rounded-full flex items-center cursor-pointer p-1 shadow-inner transition-all duration-300"
          @click="toggleDifficulty">
          <div
            class="absolute h-full text-sm w-1/2 px-8 bg-gradient-to-r from-sky-500 to-sky-600 text-white font-semibold flex items-center justify-center rounded-full transition-all duration-300 ease-in-out"
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div v-for="board in boardStore.boards" :key="board.id"
          ...
          <div class="mt-8">
            ...
            <RouterLink v-else-if="!authStore.user" :to="{ name: 'login' }"
              class="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600 transition mt-8">
              Login
            </RouterLink>
    ...
    <!-- Tabela de Jogos -->
    <div v-if="authStore.user" class="pt-8">
      <div>
      ...
      <div class="bg-white p-6 rounded-lg shadow-md mt-6">
        ...
        <p class="text-xs text-gray-500">
          1. <strong>Best Time</strong> - The fastest time you achieved for completing games on each
          board size. <br />2. <strong>Turns</strong> - The total number of moves you made to
          complete a game on each board size.
        </p>
      </div>
    </div>
  </div>
